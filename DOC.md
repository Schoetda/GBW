...
\-- Settings (hierarchical menu navigation)
    ...
    |   |-- Start on Cup toggle (auto-start when target cup lands within the threshold window)
    |   \-- Return on Removal toggle (auto-close completion once the cup leaves the scale)
    |
    +-- Tools
    |   |-- Tare button
    |   |-- Calibrate button
